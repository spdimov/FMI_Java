...
public class SpotifyExplorer {
    ...
    public SpotifyExplorer(Reader dataInput) {
        ...
        ReadFile.readFromFileToList(dataInput, trackList);
    }
    ...
    public Collection<SpotifyTrack> getAllSpotifyTracks() {
        return Collections.unmodifiableCollection(trackList);
    }
    ...
    public int getArtistActiveYears(String artist) {
        ...
        IntSummaryStatistics stats = trackList.stream()
    ...
    public List<SpotifyTrack> getTopNHighestValenceTracksFromThe80s(int n) {
        ...
        return trackList.stream()
                .filter(track -> track.year() <= 1989 && track.year() >= 1980)
    ...
    public SpotifyTrack getMostPopularTrackFromThe90s() {
        return trackList.stream()
                .filter(track -> track.year() <= 1999 && track.year() >= 1990)
    ...
    public long getNumberOfLongerTracksBeforeYear(int minutes, int year) {
        final double MILISECONDS_IN_MINUTE = 60000;
        if (minutes < 0 || year < 0) {
            throw new IllegalArgumentException();
        }
        ...
        return trackList.stream()
                ...
                .filter(track -> track.duration() / MILISECONDS_IN_MINUTE > minutes)
                .count();
    }
